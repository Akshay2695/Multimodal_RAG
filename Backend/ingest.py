...
#     if not pdf_files:
#         logger.error(f"No PDF files found in {source_directory}")
#         return
...
#     if not all_texts and not all_tables:
#         logger.error("No content extracted from PDFs")
#         return

#     # Process images
#     images_b64 = get_images_base64(all_texts)
    
#     # Get summaries
#     text_summaries, table_summaries = summarize_texts_and_tables(all_texts, all_tables)
#     image_summaries = summarize_images(images_b64) if images_b64 else []
...
#                             for summary, img_id in zip(image_summaries, img_ids)]
